...
def register_instruction(funct3, reg, reg2, result_reg, special):
    ...
    special = special & 1
    opcode = 0b0110011 
    instruction = opcode | (rs2 << 20) | (rd << 7) | (rs1 << 15) | (funct3 << 12) | (special << 30)
    return instruction & 0xffffffff

def add(reg, reg2, result_reg):
    return register_instruction(funct3=0, reg=reg, reg2=reg2, result_reg=result_reg, special=0)

def sub(reg, reg2, result_reg):
    return register_instruction(funct3=0, reg=reg, reg2=reg2, result_reg=result_reg, special=1)

def sll(reg, reg2, result_reg):
    return register_instruction(funct3=1, reg=reg, reg2=reg2, result_reg=result_reg, special=0)

def slt(reg, reg2, result_reg):
    return register_instruction(funct3=2, reg=reg, reg2=reg2, result_reg=result_reg, special=0)

def sltu(reg, reg2, result_reg):
    return register_instruction(funct3=3, reg=reg, reg2=reg2, result_reg=result_reg, special=0)

def xor(reg, reg2, result_reg):
    return register_instruction(funct3=4, reg=reg, reg2=reg2, result_reg=result_reg, special=0)

def srl(reg, reg2, result_reg):
    return register_instruction(funct3=5, reg=reg, reg2=reg2, result_reg=result_reg, special=0)

def sra(reg, reg2, result_reg):
    return register_instruction(funct3=5, reg=reg, reg2=reg2, result_reg=result_reg, special=1)

def or_(reg, reg2, result_reg):
    return register_instruction(funct3=6, reg=reg, reg2=reg2, result_reg=result_reg, special=0)
...
def branch(funct3, immediate, reg, reg2):
    ...
    instruction = get_slice(immediate, 12, 12) << 31 | get_slice(immediate, 10, 5) << 25 | get_slice(immediate, 4, 1) << 8 | get_slice(immediate, 11, 11) << 7
