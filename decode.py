...
def decode(instruction, src1, src2, fwd1, fwd2, fwd_val, this_pc):
    ...
    B            = Signed(32).select(opcode_is_immediate, src2, Signed(32).constant(0) + instruction[31:20])
...
if __name__ == "__main__":
    ...
    src1_out, src2_out, A, B, operation, shift_amount, add_sub, issigned = decode(*inputs)
    ...
    for opcode in opcodes:
        for funct3 in [0, 1, 6, 7, 0x7ffffffe, 0x7fffffff, 0x80000000, 0xfffffffe, 0xffffffff]:
            for rs1 in [0, 31]:
                for rs2 in [0, 31]:
                    for signed in [0, 1]:
                        instruction_stim.append(opcode | (rs1 << 15) | (rs2 << 20) | (funct3 << 12) | (signed << 30))
    ...
    stimulus = itertools.product(instruction_stim, src1_stim, src2_stim, fwd1_stim, fwd2_stim, fwd_val_stim, this_pc_stim)
    ...
    for idx, stim in enumerate(stimulus):
        ...
        expected_src1, expected_src2, expected_A, expected_B, expected_operation, expected_shift_amount, expected_add_sub, expected_signed = decode_model(*stim)
